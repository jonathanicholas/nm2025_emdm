...
parameters {
    // Hierarchical decay rate
    real<lower=0> decay_rate_mu;
    real<lower=0> decay_rate_sigma;
    vector<lower=0>[n_subjects] decay_rate_raw;
...
transformed parameters {
    ...
    vector<lower=0>[n_subjects] decay_rate;
    ...
    decay_rate = decay_rate_mu + decay_rate_sigma * decay_rate_raw;
    ...
    // Process all learning trials to compute feature sums with individual decay rates
    for (t in 1:n_learning_trials) {
        ...
        // Use subject-specific decay rate
        real weight = exp(-decay_rate[s] * (6 - trial_position[t]));
        real weighted_outcome = outcome[t] * weight;
        
        feature_sums[s,g,f1] += weighted_outcome;
        feature_sums[s,g,f2] += weighted_outcome;
    }
...
model {
    // Hierarchical decay rate priors
    decay_rate_mu ~ exponential(1);
    decay_rate_sigma ~ exponential(1);
    decay_rate_raw ~ normal(0, 1);
